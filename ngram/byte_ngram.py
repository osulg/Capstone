...
# 모든 결과를 여기 하나로 통일
BASE_OUTPUT_DIR = Path("~/capstone/Result/byte_ngram").expanduser()
ALL_OUTPUT_DIR = BASE_OUTPUT_DIR

EXPERIMENT_TAG = f"n{N_GRAM}_k{TOP_K}"

COMBINED_CSV = ALL_OUTPUT_DIR / f"byte_ngram_{EXPERIMENT_TAG}_all_dataset.csv"
GLOBAL_VOCAB_JSON = BASE_OUTPUT_DIR / f"global_vocab_{EXPERIMENT_TAG}.json"

# family 폴더 탐색 시 제외 폴더
EXCLUDE_DIRS = {"byte_ngram", "__pycache__", "all", "PE"}
...
def get_byte_ngram_counts(file_path: Path, n=3):
    """
    파일을 바이너리로 읽고 byte n-gram의 Counter를 반환

    Args:
        file_path (Path): 분석 파일 경로
        n (int): n-gram 길이

    Returns:
        Counter:
            key   = n바이트 패턴(bytes 형태)
            value = 해당 패턴의 등장 횟수
    """
    ...
    # 파일 길이가 n보다 짧으면 -> n-gram 만들 수 없음
    if len(data) < n:
        counts = Counter()
    else:
        # 길이 n 바이트 조각을 한 칸씩 밀면서 생성
        # ex) n=3 - [0:3], [1:4], [2:5]
...
# =========================
def build_global_vocab(family_to_files: dict, n: int = 3, top_k: int = 1000):
    """
    - 모든 family의 모든 파일 대상
    - n-gram으로 빈도 누적
    ...
    for family, file_list in family_to_files.items():
        ...
        for file_path in file_list:
            try:
                # 파일 1개의 n-gram 빈도 얻기
                file_counter = get_byte_ngram_counts(file_path, n)
                # 전체 누적 Counter에 더함
...
def file_to_vector(file_path: Path, vocab, n: int = 3):
    ...
    """
    count = get_byte_ngram_counts(file_path, n)
...
def save_vocab_json(vocab, json_path: Path):
    ...
    """
    vocab_mapping = {f"f_{i}": gram.hex() for i, gram in enumerate(vocab)}
